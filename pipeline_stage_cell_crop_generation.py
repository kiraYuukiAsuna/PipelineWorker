...
class DataAccess:
    ...
    def get_soma_list(self, ptrsid, docid):
        ...
        """
        apo_file = os.path.join(self.soma_dir, ptrsid, docid, docid + '_initial.apo')
...
def main(h5_image_name):
    ...
    csv_file_path = os.path.join(cfg.SamplePreparationDirectory, ptrsid, docid, "cell_with_cellID.csv")
    ...
    if somalist is None:
        print("Soma info not found for:", docid)
    ...
    if img is None:
        print("Image data not found for:", docid)
    ...
    for s in somalist.index:
        ...
        if x_start >= x_end or y_start >= y_end:
            print("Invalid crop region for cell_id:", this_sid, "at x,y =", sx, sy)
            continue
        ...
        if not os.path.exists(out_file):
            ...
            out_mip_dir = os.path.join(cfg.Cell_MIPsDirectory, fp2dbdirs(this_sid))
...
if __name__ == '__main__':
    # python3 {script_path} --image-path "{image_path} --cell-id-begin {cell_id_begin} --cell-id-end {cell_id_end}"
    # 解析参数 --image-path
    import argparse
    parser = argparse.ArgumentParser(description="crop cell images from v3d files.")
